...
class CoverPage(Flowable):
    ...
    def draw(self):
        ...
        y -= 0.5*cm
        meta = [
            ("Referencia catastral", d.get('ref_catastral', '—')),
            ("Registro de la Propiedad", d.get('registro', '—')),
            ("Municipio", d.get('municipio', '—')),
            ("Fecha del informe", d.get('fecha', datetime.date.today().strftime('%d de %B de %Y'))),
            ("Elaborado por", T(d.get("lang","sv"), "elaborado")),
            ("Alcance", T(d.get("lang","sv"), "alcance")),
        ]
...
def generate_pdf(report_data: dict, output_path: str):
    ...
    rows_id = [
        ('Dirección', prop.get('direccion', '—')),
        ('Referencia catastral', prop.get('ref_catastral', '—')),
        ('Tipo de inmueble', prop.get('tipo', '—')),
        ('Municipio', prop.get('municipio', '—')),
        ('Año de construcción', prop.get('anno_construccion', '—')),
        ('Precio de compraventa', prop.get('precio', '—')),
        ('Vendedor', prop.get('vendedor', '—')),
        ('Estado actual', prop.get('estado', '—')),
    ]
    ...
    rows_reg = [
        ('Registro de la Propiedad', reg.get('registro', prop.get('registro', '—'))),
        ('N.º de finca', reg.get('finca', '—')),
        ('Superficie inscrita (construida)', f"{reg.get('superficie_registro', '—')} m²" if reg.get('superficie_registro') else '—'),
        ('Superficie inscrita (parcela)', f"{reg.get('superficie_parcela_registro', '—')} m²" if reg.get('superficie_parcela_registro') else '—'),
        ('Titularidad', reg.get('titularidad', '—')),
        ('Coordinación con Catastro', reg.get('coordinacion_catastro', '—')),
    ]
    ...
    cargas_text = {
        ...
        'hipoteca': 'La finca esta gravada con hipoteca. Ver analisis de riesgos.',
        'embargo': 'La finca presenta anotacion de embargo. Ver analisis de riesgos.',
        'usufructo': 'La finca está sujeta a derecho de usufructo. Ver análisis de riesgos.',
        'unknown': 'No se ha facilitado la nota simple. Consulta urgente del Registro recomendada.',
    ...
    rows_cat = [
        ...
        ('Superficie construida', f"{cat.get('superficie_catastro', '—')} m²" if cat.get('superficie_catastro') else '—'),
        ('Superficie de parcela', f"{cat.get('superficie_parcela_catastro', '—')} m²" if cat.get('superficie_parcela_catastro') else '—'),
        ('Año de construcción', cat.get('anno_construccion_catastro', '—')),
        ('Valor catastral', f"{cat.get('valor_catastral', '—')} €" if cat.get('valor_catastral') else '—'),
        ('Valor de referencia (AEAT)', f"{cat.get('valor_referencia', '—')} €" if cat.get('valor_referencia') else '—'),
        ('Piscina en Catastro', 'Sí' if cat.get('piscina_catastro') else ('No' if cat.get('piscina_catastro') is False else '—')),
        ('Garaje en Catastro', 'Sí' if cat.get('garaje_catastro') else ('No' if cat.get('garaje_catastro') is False else '—')),
    ]
    ...
    if sup_cat and sup_reg and abs(float(sup_cat) - float(sup_reg)) > 5:
        ...
        story.append(Paragraph(
            f'Existe una divergencia de {diff:.0f} m² entre la superficie inscrita en el Registro '
            f'({sup_reg} m²) y la reflejada en el Catastro ({sup_cat} m²). '
            'Las construcciones adicionales pueden no constar inscritas registralmente, '
            'lo que requiere declaración de obra nueva antes de la escritura.',
            S_BODY))

    # ── S04 SITUACIÓN URBANÍSTICA ────────────────────────────────────────────
    story.extend(section_heading('04', T(lang, 's04')))
    rows_urb = [
        ('Clasificación del suelo', urb.get('tipo_suelo', prop.get('tipo_suelo', '—'))),
        ('Zona costera (Ley de Costas)', 'Sí — revisar deslinde' if prop.get('zona_costera') else 'No indicado'),
        ('Licencia de primera ocupación', prop.get('tiene_licencia_primera_ocupacion', '—')),
        ('Expediente urbanístico', prop.get('expediente_urbanistico', '—')),
    ]
